...
/* =====================================================================
 *  Battle Logger — records online battle history to .log files
 * ===================================================================== */

#define BLOG_CAP_INIT 8192

typedef struct {
    char *buf;          /* dynamically grown text buffer */
    int   len;
    int   cap;
    int   active;       /* 1 = logging enabled for this game */
    int   saved;        /* 1 = already written to disk */
    char  room_id[ROOM_ID_MAX + 1];
    char  my_role[4];   /* "A" or "B" */
    char  my_color[10]; /* "Red" or "Black" */
    int   move_num;     /* move counter */
} BattleLog;
                                ...
                                int old_total, const Cell prev[][BOARD_COLS])
{
    ...
    /* Update color if just determined */
...
static void blog_action_sent(const char *action) { (void)action; }
...
static void blog_game_over(const GameState *gs)
{
    ...
    /* Write to file: logs/battle_YYYYMMDD_HHMMSS_<room>.log */
    time_t now = time(NULL);
    struct tm *t = localtime(&now);
    char filename[256];
...
int gui_main(int auto_online, const char *auto_room_id)
{
    ...
    while (!WindowShouldClose()) {
                ...
                if (conn_frame == 0) {
                    ...
                    if (net_connect(NET_SERVER_IP, NET_SERVER_PORT) != 0) {
                        ...
                        if (net_join_room(room_input, role,
                            ...
                            blog_start(room_input, my_role_ab);
                        }
                    }
                }
            }
        ...
        if (online_mode) {
            ...
            if (!gs.game_over) {
                ...
                while ((recv_result = net_try_receive(
                    ...
                    if (net_parse_update(recv_buf, &gs, &net_info)) {
                        waiting_for_update = 0;
                        if (online_ai_mode) ai_delay = 1; /* 2s delay */
                        ...
                        if (net_info.total_moves != old_total &&
                            ...
                            /* Track no_capture_turns and reconstruct last_move */
                            if (tr > 0) {
                        ...
                        if (strcmp(net_info.state, "playing") != 0 &&
                            ...
                            if (strcmp(net_info.winner_role, "A") == 0) {
                                ...
                                /* Server didn't send winner field — derive from
                                 * alive counts (one side wiped out = that side lost) */
                                if (gs.red_alive > 0 && gs.black_alive == 0)
                                    gs.winner = SIDE_RED;
                                else if (gs.black_alive > 0 && gs.red_alive == 0)
                                    gs.winner = SIDE_BLACK;
                                else
                                    gs.winner = SIDE_NONE;  /* genuine draw */
                            }
                            blog_game_over(&gs);
                        }
                    }
            ...
            /* Bottom-left: show AI mode (NORMAL / DRAW) */
            if (online_ai_mode && sides_determined) {
                const char *mode_label = strategy_is_draw_mode() ? "DRAW" : "NORMAL";
                Color mode_col = strategy_is_draw_mode()
                    ? (Color){180, 140, 0, 255}   /* 黃色：平局模式 */
                    : (Color){80, 160, 80, 255};  /* 綠色：正常模式 */
                Vector2 mode_sz = MeasureTextEx(g_font, mode_label,
                                                (float)FONT_SIZE_SMALL, 1);
                Vector2 mode_pos = {
                    8,
                    WIN_H - STATUS_HEIGHT / 2 - mode_sz.y / 2
                };
                DrawTextEx(g_font, mode_label, mode_pos,
                           (float)FONT_SIZE_SMALL, 1, mode_col);
            }

            /* Bottom-right: show our side */
            if (sides_determined) {
                const char *side_label = (player_side == SIDE_RED) ? "RED" : "BLACK";
                Color side_col = (player_side == SIDE_RED)
                    ? (Color){200, 50, 50, 255} : (Color){60, 60, 60, 255};
                Vector2 side_sz = MeasureTextEx(g_font, side_label,
                                                (float)FONT_SIZE_SMALL, 1);
                Vector2 side_pos = {
                    WIN_W - side_sz.x - 8,
                    WIN_H - STATUS_HEIGHT / 2 - side_sz.y / 2
                };
                DrawTextEx(g_font, side_label, side_pos,
                           (float)FONT_SIZE_SMALL, 1, side_col);
            }
