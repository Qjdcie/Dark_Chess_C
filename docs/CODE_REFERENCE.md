...
> **版本：** 2026-04-26
> **涵蓋原始碼：** `src/core/board.h`、`src/core/board.c`、`src/core/strategy.h`、`src/core/strategy.c`、`src/core/net_client.h`、`src/core/net_client.c`
...
5. [座標系統（必讀）](#5-座標系統必讀)
6. [網路模組（net_client）](#6-網路模組)
7. [策略模組架構（strategy.c）](#7-策略模組架構)
8. [全域狀態變數](#8-全域狀態變數)
9. [資料結構詳解](#9-資料結構詳解)
10. [工具函數](#10-工具函數)
11. [動態棋子價值系統](#11-動態棋子價值系統)
12. [算牌系統](#12-算牌系統)
13. [威脅偵測函數](#13-威脅偵測函數)
14. [砲線分析函數](#14-砲線分析函數)
15. [吃子分析函數](#15-吃子分析函數)
...
│   │   ├── net_client.h   ← 網路對戰模組對外介面（連線、收發、JSON 解析）
│   │   └── net_client.c   ← 網路對戰實作（TCP 連線、房間加入、棋盤同步）
│   │
│   ├── gui/
...
gui_main.c
    ...
    ├── net_client.h / net_client.c  (網路對戰，內部引用 board.h)
    └── gui/raylib_gui.h / .c    (畫面顯示，引用 board.h + net_client.h)
```

**strategy.c 只能讀取 GameState，不能修改它。** 所有走法透過
`board_apply_move()` 由外部執行，strategy 只負責「選哪步」。

**net_client.c 負責與伺服器通訊**，將伺服器的 JSON 更新解析為
`GameState` 和 `NetUpdateInfo`，GUI 根據這些資訊更新畫面和判定勝負。
...
> 定義於 `src/core/net_client.h` 與 `src/core/net_client.c`。
> 負責與對戰伺服器的 TCP 連線、JSON 解析、棋盤狀態同步。

### 6.1 結構

#### `NetUpdateInfo` — 伺服器更新資訊
...
| 欄位 | 說明 |
|------|------|
| `current_turn_role` | 目前輪到哪位玩家（`"A"` 或 `"B"`） |
| `state` | 遊戲狀態（`"waiting"`、`"playing"`、結束時為其他值） |
| `color_a` / `color_b` | 玩家 A/B 的陣營（`"Red"`、`"Black"`、`"None"`） |
| `winner_role` | 勝方角色（`"A"`、`"B"`、或空字串），用於判定遊戲結果 |
| `total_moves` | 伺服器記錄的總走法數 |
| `player_count` | 房間內玩家數 |

### 6.2 API 函數

#### `net_connect(ip, port)` — 連線至伺服器

```c
int net_connect(const char *ip, int port);
// 回傳：0 = 成功，-1 = 失敗
// 預設伺服器：NET_SERVER_IP (140.124.184.220)，NET_SERVER_PORT (8888)
```

---

#### `net_join_room(room_id, role_out, role_len)` — 加入房間

```c
int net_join_room(const char *room_id, char *role_out, int role_len);
// room_id：房間 ID 字串
// role_out：回傳角色（"first" 或 "second"）
// 回傳：0 = 成功，-1 = 失敗
// 阻塞直到伺服器回應
```
...
#### `net_send_action(action)` — 發送走法

```c
void net_send_action(const char *action);
// action 格式："row col\n"（翻棋）或 "from_r from_c to_r to_c\n"（走/吃）
```

---

#### `net_try_receive(buffer, len)` — 非阻塞接收

```c
int net_try_receive(char *buffer, int len);
// 回傳：> 0 = 讀取的位元組數，0 = 無資料，-1 = 斷線/錯誤
// 內部緩衝不完整訊息，只回傳完整的 UPDATE 訊息
```

---

#### `net_parse_update(raw, gs, info)` — 解析伺服器 JSON

```c
int net_parse_update(const char *raw, GameState *gs, NetUpdateInfo *info);
// raw：包含 "UPDATE {json...}" 的原始字串
// gs：輸出 — 填充棋盤狀態（cells、red_alive、black_alive）
// info：輸出 — 填充 NetUpdateInfo 各欄位
// 回傳：1 = 成功，0 = 解析失敗
```

**注意：** 此函數不設定 `gs->game_over` 和 `gs->winner`，
這兩個欄位由 GUI 層根據 `info->state` 和 `info->winner_role` 判定。
...
GUI 層（`raylib_gui.c`）透過以下邏輯判定線上對戰結果：

```c
// 當伺服器回報遊戲結束（state 不是 "playing" 也不是 "waiting"）
if (strcmp(info->state, "playing") != 0 &&
    strcmp(info->state, "waiting") != 0) {
    gs.game_over = 1;

    // 根據 winner_role 和 color_table 判定勝方
    if (strcmp(info->winner_role, "A") == 0)
        gs.winner = (color_a == "Red") ? SIDE_RED : SIDE_BLACK;
    else if (strcmp(info->winner_role, "B") == 0)
        gs.winner = (color_b == "Red") ? SIDE_RED : SIDE_BLACK;
    else
        gs.winner = SIDE_NONE;  // 和棋
}
```

**重要：** 線上模式的勝負**完全由伺服器決定**，不使用本地的
`board_check_game_over()`，因為蓋牌棋子對客戶端不可見。
...
威脅清單、殺將機會等。詳見 [§16 情勢評估系統](#16-情勢評估系統)。
